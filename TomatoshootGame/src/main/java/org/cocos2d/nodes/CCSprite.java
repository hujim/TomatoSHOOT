 ...
 *
 */
public class CCSprite extends CCNode implements CCRGBAProtocol, CCTextureProtocol, Serializable {
    ...
    */
    ...
    protected void init() {
        ...
        colors    = BufferUtils.newFloatBuffer(4 * 4); //default 4*4
    ...
    public void updateColor() {		
        ...
        try {
			colors.put(tmpR).put(tmpG).put(tmpB).put(tmpA).put(tmpR).put(tmpG)
					.put(tmpB).put(tmpA).put(tmpR).put(tmpG).put(tmpB)
					.put(tmpA).put(tmpR).put(tmpG).put(tmpB).put(tmpA);
			colors.position(0);
		} catch (Exception e) {
