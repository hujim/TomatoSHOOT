...
public class GameLayerLV03 extends GameLayerBase
{
	...
	Scripts[][] scriptListsDepth= {
			...
			{Scripts.TOWARD,Scripts.AWAY,Scripts.TOWARD,Scripts.AWAY,Scripts.TOWARD}
	};
	...
	public static CCScene scene()
	{
		scene = CCScene.node();
		scene.setTag(2);
		CCColorLayer layer = new GameLayerLV03(ccColor4B.ccc4(0, 0, 0, 0));
		//scene.setVisible(false);
		scene.addChild(layer);
		
		return scene;
	}
	protected GameLayerLV03(ccColor4B color)
	{
		super(color);		
	...
	public void initContents()
	{
		...
		addBugs(EnemyTypes.BOTTLEA, PathTypes.A, finalHP_A,0,2);
		addBugs(EnemyTypes.BOTTLEA, PathTypes.A, finalHP_A,0,2);
		addBugs(EnemyTypes.BOTTLEA, PathTypes.A, finalHP_A,0,2);
		addBugs(EnemyTypes.BOTTLEA, PathTypes.A, finalHP_A,0,2);
		addBugs(EnemyTypes.BOTTLEA, PathTypes.A, finalHP_A,0,2);
		
		addBugs(EnemyTypes.BOTTLEB, PathTypes.B, finalHP_B,0,3);
		addBugs(EnemyTypes.BOTTLEB, PathTypes.B, finalHP_B,0,3);
		addBugs(EnemyTypes.BOTTLEB, PathTypes.B, finalHP_B,0,3);
		addBugs(EnemyTypes.BOTTLEB, PathTypes.B, finalHP_B,0,3);
		addBugs(EnemyTypes.BOTTLEB, PathTypes.B, finalHP_B,0,3);
	...
	protected void addBugs(EnemyTypes etype, PathTypes ptype, float eHP, int ezSPD, int exySPD)
	{
		...
		if (etype==EnemyTypes.BOTTLEA)
			target.bugSprite = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("bottleA.png"));
		else
			target.bugSprite = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("bottleB.png"));
