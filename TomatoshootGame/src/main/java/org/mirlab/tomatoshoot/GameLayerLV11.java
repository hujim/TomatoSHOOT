...
public class GameLayerLV11 extends GameLayerBase
{
	...
	public static CCScene scene()
	{
		scene = CCScene.node();
		scene.setTag(2);
		CCColorLayer layer = new GameLayerLV11(ccColor4B.ccc4(0, 0, 0, 0));
		//scene.setVisible(false);
		scene.addChild(layer);
		
		return scene;
	}
	protected GameLayerLV11(ccColor4B color)
	{
		super(color);		
	...
	public void initContents()
	{
		...
		for(int i=1; i<=20; i++){
			addBugs(EnemyTypes.BALLOON, PathTypes.A, finalHP_A,0,8);
		}
	...
	protected void addBugs(EnemyTypes etype, PathTypes ptype, float eHP, int ezSPD, int exySPD)
	{
		...
		target.bugSprite = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("BALLOON.png"));
