...
public class GameStartLayer extends CCColorLayer
{
	...
	public void initContents()
	{
		...
		//isFirstRun = (tomatoFighter != null)?true:false;
		...
		*/
		if(isFirstRun)
		{
			mainMenu = CCMenu.menu(newGame,setting,help,about);
		}
		else
		{
		...
		//fbMenu.setAnchorPoint(FB_btn.getAnchorPoint());
		FB_btn.runAction(CCRepeatForever.action(
				CCSequence.actions(
						CCFadeOut.action(0.1f),
						CCFadeIn.action(0.1f),
						CCFadeOut.action(0.1f),
						CCFadeIn.action(0.1f),						
						CCDelayTime.action(2.3f)
				)));
		CCIntervalAction move = CCMoveBy.action(0.1f, CGPoint.make(0,winSize.height*0.5f));
        CCIntervalAction move2 = CCMoveBy.action(1f, CGPoint.make(0,winSize.height*0.5f));
    
        CCIntervalAction move_ease = CCEaseBounceIn.action(move2.copy());
        CCIntervalAction move_ease_back = move_ease.reverse();
		FB_btn.runAction(CCRepeatForever.action(
				CCSequence.actions(
						//CCJumpBy.action(1f, CGPoint.ccp(0, 0), 100, 5),
						move,
						move_ease_back,
						CCDelayTime.action(8f)
				)));
		addChild(fbMenu,100);
		/*
		...
		*/
		...
		CCSprite tomato = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("tomato_head.png"));
		tomato.setPosition(winSize.width*0.4f, winSize.height*0.9f);
		tomato.setScale(winSize.height/(4.5f*title.getContentSize().height));		
		tomato.setRotation(-15);	
		tomato.runAction(CCRepeatForever.action(
				CCSequence.actions(
		...
		ship.setPosition(ship.getContentSize().width*1.1f, 0);//200
		ship.setRotation(90);
		...
		shipfire.setPosition(ship.getContentSize().width*1.0f,ship.getContentSize().height*0.2f);
	...
	{		
		...
		else if(item.getTag() == 2 )  //quick play
		{
			if(isFirstRun)
			{
				((TomatoshootGame)CCDirector.sharedDirector().getActivity()).toastMessage("請先new game製造角色~");
			}
			else
			{
				((TomatoshootGame)CCDirector.sharedDirector().getActivity()).isQuickGame = true;
				CCDirector.sharedDirector().replaceScene(CCFlipAngularTransition.transition(0.5f, SetGameItemLayer.scene(), tOrientation.kOrientationRightOver));
				//((TomatoshootGame)CCDirector.sharedDirector().getActivity()).inTransition = true;
		...
		else if(item.getTag() == 5 )  //about
		{
			...
			//((TomatoshootGame)CCDirector.sharedDirector().getActivity()).toastMessage("ABOOUT!");
			CCDirector.sharedDirector().pushScene(AboutLayer.scene());
		}
...
//			}
			friendItem[i] = CCMenuItemSprite.item(
					((TomatoshootGame)CCDirector.sharedDirector().getActivity()).friendSprite[i], 
					((TomatoshootGame)CCDirector.sharedDirector().getActivity()).friendSprite[i], this, null);
