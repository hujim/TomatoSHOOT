...
public class GameLayerLV01 extends GameLayerBase
{
	...
	public static CCScene scene()
	{
		scene = CCScene.node();
		scene.setTag(2);
		CCColorLayer layer = new GameLayerLV01(ccColor4B.ccc4(0, 0, 0, 0));
		//scene.setVisible(false);
		scene.addChild(layer);
		
		return scene;
	}
	protected GameLayerLV01(ccColor4B color)
	{
		super(color);		
	...
	public void initContents()
	{
		...
		*/
		...
		addBugs(EnemyTypes.BULLEYE, PathTypes.A, finalHP_A,0,0);
		addBugs(EnemyTypes.BULLEYE, PathTypes.A, finalHP_A,0,0);
		addBugs(EnemyTypes.BULLEYE, PathTypes.A, finalHP_A,0,0);
		addBugs(EnemyTypes.BULLEYE, PathTypes.A, finalHP_A,0,0);
		addBugs(EnemyTypes.BULLEYE, PathTypes.A, finalHP_A,0,0);
	...
	protected void addBugs(EnemyTypes etype,PathTypes ptype, float eHP, int ezSPD, int exySPD)
	{
		...
		//if (etype==EnemyTypes.BUGA)
			target.bugSprite = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("bullEye.png"));
		...
		target.distance = rand.nextInt(100)+200;
