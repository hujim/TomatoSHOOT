...
public class GameLayerLV07 extends GameLayerBase
{
	...
	public static CCScene scene()
	{
		scene = CCScene.node();
		scene.setTag(2);
		CCColorLayer layer = new GameLayerLV07(ccColor4B.ccc4(0, 0, 0, 0));
		//scene.setVisible(false);
		scene.addChild(layer);
		
		return scene;
	}
	protected GameLayerLV07(ccColor4B color)
	{
		super(color);		
	...
	public void initContents()
	{
		...
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,1,3);
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,1,3);
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,2,2);
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,2,2);
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,3,1);
		addBugs(EnemyTypes.STRAW, PathTypes.A, finalHP_A,3,1);
	...
	protected void addBugs(EnemyTypes etype, PathTypes ptype, float eHP, int ezSPD, int exySPD)
	{
		...
		target.bugSprite = CCSprite.sprite(CCSpriteFrameCache.sharedSpriteFrameCache().spriteFrameByName("STRAW.png"));
